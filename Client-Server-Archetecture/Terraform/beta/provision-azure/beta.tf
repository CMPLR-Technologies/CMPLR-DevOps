...
resource "azurerm_resource_group" "beta_rg" {
    name     = "beta_rg"
    location = "eastus"
...
resource "azurerm_virtual_network" "beta_vnet" {
    name                = "beta_vnet"
...
# Create subnet
resource "azurerm_subnet" "beta_subnet" {
    name                 = "beta_subnet"
    resource_group_name  = azurerm_resource_group.beta_rg.name
    virtual_network_name = azurerm_virtual_network.beta_vnet.name
    address_prefixes       = ["10.0.1.0/24"]
}

# Create public IPs
resource "azurerm_public_ip" "beta_ip" {
    name                         = "beta_ip"
    location                     = "eastus"
    resource_group_name          = azurerm_resource_group.beta_rg.name
...
resource "azurerm_network_security_group" "beta_sg" {
    name                = "beta_sg"
    location            = "eastus"
    resource_group_name = azurerm_resource_group.beta_rg.name
...
resource "azurerm_network_interface" "beta_NIC" {
    name                      = "beta_NIC"
    location                  = "eastus"
    resource_group_name       = azurerm_resource_group.beta_rg.name
    ...
    ip_configuration {
        ...
        subnet_id                     = azurerm_subnet.beta_subnet.id
        private_ip_address_allocation = "Dynamic"
        public_ip_address_id          = azurerm_public_ip.beta_ip.id
    }
...
resource "azurerm_network_interface_security_group_association" "example" {
    network_interface_id      = azurerm_network_interface.beta_NIC.id
    network_security_group_id = azurerm_network_security_group.beta_sg.id
}
...
resource "random_id" "randomId" {
    keepers = {
        ...
        resource_group = azurerm_resource_group.beta_rg.name
    }
...
resource "tls_private_key" "beta_key" {
  algorithm = "RSA"
  rsa_bits = 4096
}
output "tls_private_key" { 
    value = tls_private_key.beta_key.private_key_pem 
    sensitive = true
}

# Create virtual machine
resource "azurerm_linux_virtual_machine" "betamaster" {
    name                  = "betamaster"
    location              = "eastus"
    resource_group_name   = azurerm_resource_group.beta_rg.name
    ...
    computer_name  = "betamaster"
    admin_username = "azureuser"
    disable_password_authentication = true

    admin_ssh_key {
        username       = "azureuser"
        public_key     = tls_private_key.beta_key.public_key_openssh
    }
...
resource "null_resource" "beta-null" {
 depends_on = [
    azurerm_public_ip.beta_ip,
   azurerm_linux_virtual_machine.betamaster
 ] 
...
# Copy in the bash script we want to execute.
  ...
  connection {
    type    = "ssh"
    user = "azureuser"
    private_key = tls_private_key.beta_key.private_key_pem
    host = azurerm_linux_virtual_machine.betamaster.public_ip_address
  }
