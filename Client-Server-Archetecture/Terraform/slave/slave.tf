...
resource "azurerm_network_security_group" "cmplr_sg" {
    ...
    security_rule {
        name                       = "apache"
        priority                   = 1002  //ports with the same direction can't have the same priority
        direction                  = "Inbound"
        access                     = "Allow"
        protocol                   = "Tcp"
        source_port_range          = "*"
...
resource "azurerm_linux_virtual_machine" "cmplrslave" {
    name                  = "cmplrslave"
    location              = "eastus"
    resource_group_name   = azurerm_resource_group.cmplr_rg.name
...
# Copy in the bash script we want to execute.
  ...
  connection {
    ...
    host = azurerm_linux_virtual_machine.cmplrslave.public_ip_address
  }
